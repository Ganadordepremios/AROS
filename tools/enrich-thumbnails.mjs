...
async function instagramThumb(url) {
  // 1) Meta oEmbed oficial (si hay token)
  if (META_TOKEN) {
    try {
  ...
  // 2) Fallback: og:image del /embed/ público
  try {
    const m = url.match(/instagram\.com\/(?:p|reel|reels|tv)\/([\w-]+)/);
    if (!m) return null;
    const r = await fetch(`https://www.instagram.com/p/${m[1]}/embed/captioned/`, { headers: { 'User-Agent': UA, 'Accept-Language': 'es,en' } });
    if (!r.ok) return null;
    const html = await r.text();
    let og = html.match(/property=["']og:image["']\s+content=["']([^"']+)["']/i) || html.match(/content=["']([^"']+)["']\s+property=["']og:image["']/i);
    if (og) return og[1].replace(/&amp;/g, '&');
    const du = html.match(/"display_url":"([^"]+)"/);
    if (du) return du[1].replace(/\\u0026/g, '&').replace(/\\\//g, '/');
    return null;
  } catch { return null; }
}
...
async function processRow(o, linkKey) {
  ...
  if (!DRY && await head200(publicUrl(file))) { o.thumb = publicUrl(file); return { status: 'cached' }; } // ya estaba rehosteada
...
(async () => {
  ...
  console.log(`▶ ${objs.length} filas · bucket=${BUCKET} · IG=${META_TOKEN ? 'oEmbed+scrape' : 'scrape'} ${DRY ? '· DRY-RUN' : ''}`);
  await ensureBucket();
